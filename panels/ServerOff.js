...
const ServerOff = props => (
	<Panel id={props.id} separator={false}>
          <Placeholder
            icon={<center><img className="clicked" src={require('../img/noconnect.png')} width={250} height='250' onClick={props.click} /></center>}
            header="Соединение потеряно"
            action={<Button onClick={props.on}>Переподключится</Button>}
            stretched
          >
            Попробуйте очистить кэш и перезапустить приложение
          </Placeholder>
	</Panel>
